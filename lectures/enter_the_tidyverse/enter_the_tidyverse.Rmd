...
- Pipes work from left to right: `LHS %>% RHS`
- The object created by `LHS` becomes the first argument of the function (`RHS`) to the right of the `%>%` pipe symbol 
- Basic flow of using pipes in code:
  - `object %>% some_function1 %>% some_function2 %>% some_function3`    
  - The output of `some_function1` becomes the input (the first argument) of the function `some_function2` to the right of the `%>%` pipe symbol 
...
```
- without pipes: first we write function, data object `1:10` is first argument of `mean` function
- with pipes: first we write data object, then we "pipe" (verb) the object as first argument of `mean` function
