---
title: "Introduction to R and R data structures"
...
__Assignment__ refers creating an "object" and assigning values to it
...
Most commercial statistical software packages (e.g., SPSS, Stata) operates on datasets, which consist of rows of observations and columns of variables
...
- Each element in a data frame must have the same length
    ...
    - Thus, each variable in data frame has same number of observations
- Each element is named
    - these element names are the variable names
- Typically, each __element__(variable) in data frame is a __vector__
...
```
The "vertices" consit of colleges and high schools
```{r}
...
- Your problem set due before class next Friday will give you more practice with filepaths and we have created a short document that may be helpful in working through this problem set [LINK](https://github.com/anyone-can-cook/rclass1/blob/master/lectures/problemset_resources/problemset_resources.pdf) 
...
1. Download this zip folder: [LINK HERE](https://github.com/anyone-can-cook/rclass1/blob/master/rclass1.zip)
...
3. Save the following files in "rclass1/lectures/intro_to_r"
    - intro_to_r.Rmd
    - intro_to_r.pdf
