...
#remove start `<a href="`  portion
jeopardy <- jeopardy %>% 
  mutate(
    links=str_sub(href_tag, 10, )
)

#remove the `">`at the end of the string
jeopardy <- jeopardy %>% 
  mutate(
    links=str_sub(links, 1, -3)
)
...
```{r}
#detect extensions at end of string
jeopardy %>% 
  select(links) %>%
  filter(str_detect(links, "\\.[:alnum:][:alnum:][:alnum:]$"))


# extract extensions 
jeopardy <- jeopardy %>% 
  mutate(
    extns=str_extract(links, "\\.([:alnum:][:alnum:][:alnum:])$")
)

#remove . from extensions using sub_replace
jeopardy <- jeopardy %>% 
  mutate(
    extns=str_replace(extns, "\\.", "")
)
...
5. Detect and extract dates from `Questions` in __any one__ format (mm/dd/yyyy, mm-dd-yyyy, yyyy/mm/dd) into a new variable called `date`. The goal here is not to get "the right answer" but get you to explore various ways to extract patterns. Don't spend too much time on this, but create a regular expression that extracts __at least__ one date observation within `date` (in whichever format).

```{r}

jeopardy %>% 
  select(Question) %>%
  filter(str_detect(Question, "(\\d{1,2}([.-/])\\d{1,2}([.-/])\\d{1,4})"))


#remove . from extensions using sub_replace
jeopardy <- jeopardy %>% 
  mutate(
    date=str_extract(Question, "(\\d{1,2}([.-/])\\d{1,2}([.-/])\\d{1,4})")
)
...
6. Using the `lubridate` library, convert the `date` variable from the previous question into a date object called `datev2`. Check the type and class of the `datev2`. FYI- if your `date` variables are not in a consistent format you may get a warning regarding how many observations `failed to parse.` 

```{r}

#some strings will not parse due to 2-digit and 4-digit years
