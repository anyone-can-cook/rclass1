...
```{r, echo=FALSE, results='hide', message=FALSE}
tidy_example <- tibble(
  person = rep(c("John Smith", "Jane Doe", "Mary Johnson"), 2),
  treatment = c(rep("treatment_a", 3), rep("treatment_b", 3)),
  result = c(NA, 16, 3, 2, 11, 1)
)
```

```{r, echo=FALSE}
kable(structure_a) %>% kable_styling(bootstrap_options = c("striped", "hover", "responsive"), full_width = F, position = "center")
```

This table shows the same data from earlier, but now the data is in **tidy** form because it satisfies the 3 rules:

1. All 3 __variables__ (`person`, `treatment`, `result`) have its own column
1. All 6 __observations__ (combination of `person`-`treatment`) have its own row
1. All 18 __values__ have its own cell in the table
...
Then, we can specify how we want to treat each part inside the `names_to` vector:
...
<br>
**Practical example** of using the `pivot_longer()` function [in the Appendix](#6_Appendix)
...
<br>
**Practical example** of using the `pivot_wider()` function [in the Appendix](#6_Appendix)
...
<br>
**Practical example** of identifying explicit and implicit missing values [in the Appendix](#6_Appendix)
...
<br>
**Practical example** of using the `complete()` function [in the Appendix](#6_Appendix)
