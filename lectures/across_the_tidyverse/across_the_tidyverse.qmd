...
## summarize() and Counts {#sec-sum-count}
...
## count() in place of group_by() and summarise()

Recall from @sec-sum-count, the `count()` function returns the count of unique values of a particular variable. For example, if we count by event_state, we get the count of visits by state. This is the same if we were to `group_by()` event_state and `summarise()` to get the count of events by state.
