...
When we provide multiple input vectors, we can see that the vectors get concatenated element-wise:
...
We can think of a string as being a character vector of size 1. For example, passing 3 strings into `str_c()` is like passing in 3 vectors of size 1 each. Remember that vectors are concatenated element-wise, so these strings will be joined:
